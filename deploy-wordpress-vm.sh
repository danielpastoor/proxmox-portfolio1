...
for i in $(seq 1 $AANTAL); do
    ...
    # Cloud-init netwerk instellen
    qm set $VMID --ipconfig0 ip=${IP},gw=${GATEWAY}
    qm set $VMID --ciuser root --cipassword 'iLn9!2dLRq8_eNhtMyLCcJ9xcy4cT'
    ...
    ssh -o StrictHostKeyChecking=no root@${VM_IP} << WPINSTALL
        ...
        # UFW firewall instellen (binnen de VM)
